...
const HeroSection = () => {
  ...
  useEffect(() => {
    ...
    const fetchSofaImage = async () => {
      try {
        const query = encodeURIComponent(
          `*[_type == "product" && name == "Luxury Flower Shell Sofa Chair"]{ "imageUrl": image.asset->url }`
        );
        ...
        if (data.result && data.result[0]?.imageUrl) {
          setSofaImageUrl(data.result[0].imageUrl);
        } else {
